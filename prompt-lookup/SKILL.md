...
## How to use the MCP Server (prompts.chat-mcp)

This skill relies on the [prompts.chat-mcp](https://github.com/f/prompts.chat-mcp) server. If the tools above are not available, it may need to be installed or configured via `mcporter`.

### Ad-hoc Setup (via mcporter)

If you need to install or run the server ad-hoc:
```bash
# Add as a persistent server
mcporter config add prompts-chat --command npx --arg -y --arg @fkadev/prompts.chat-mcp --scope home
```

### Direct Tool Usage
Once the MCP server is active, you can call its tools (`search_prompts`, `get_prompt`, `improve_prompt`) directly through the OpenClaw tool interface.
