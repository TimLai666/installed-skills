...
| `ddgs-search` | Fallback search tool | Use when `web_search` (Brave) is rate-limited or unavailable |
...
- **Rate limiting** — Slow down, reduce search count, retry with backoff; switch to `ddgs-search` if `web_search` is blocked
